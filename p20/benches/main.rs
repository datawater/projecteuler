...
fn multiply_n_polynomials(p: &[HashMap<u8, u128>]) -> HashMap<u8, u128> {
    let mut result = p[0].clone();

    for poly in &p[1..] {
        result = multiply_power_of_ten_polynomial(&result, poly);
    }
...
fn main_calculation() -> u128 {
    ...
    simplified.values().sum::<u128>()
}

fn benchmark_main(c: &mut Criterion) {
    c.bench_function("main_calculation", |b| b.iter(main_calculation));
}
